...
ICM_dev::ICM_dev()
    : Interface("ICM", 0),
      ...
      rem_setpoint(100),
      nsync(0) {
  msg(MSG_DEBUG, "%s: subbus flavor is %s", iname, ICM_SUBBUS_FLAVOR);
...
void ICM_dev::read_sensors() {
  ...
  while (mask && nsync == 0 && !quit_requested) {
    ...
    for (int i = 0; i < NS; ++i) {
      if (mask & (1<<i)) {
        ...
        if (nsync == 0) {
          ...
          if (rv < 0) {
            msg(MSG_ERROR, "%s: Driver error %d: %s", iname, rv, strerror(-rv));
            ICM20948.dev[i].status |= 4; // Mark NACK
            continue;
          }
          if (rv == SBS_NOACK) {
        ...
        if (nsync || dur.count() >= msec_setpoint || ICM20948.dev[i].mode != 2) {
          msg(MSG_DBG(1), "%s: dev[%d].nw:%d .msecs=%d nsync=%d mode=%d", iname,
...
void ICM_dev::event_loop() {
  ...
  while (!quit_requested) {
    ...
    // msg(MSG_DEBUG, "%s: Top of loop: rep_modefs = %02X",
    //   iname, rep_modefs);
    for (int i = 0; i < NS; ++i) {
