...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-focal-install.ps1
#
# ---------
# Uninstall:
# ---------
...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-focal-install.ps1 -uninstall
...
cat 1>&2 <<'EOF'
# Usage:
#   ./monarch-focal-install.sh [ -E moudi:nthallen/keutsch-moudi.git ] [-n] [-h] [-S]
#
# -E <basename>:<URL>
...
exp_option='focal:nthallen/FOCAL2023.git'
exp_base=''
exp_url=''
testmode=no
installagent=no
...
'@
$setup_script | Out-File -FilePath "./monarch-focal-install.sh" -NoNewLine -Encoding ASCII

Write-Output "`nStarting standard install script: /usr/local/src/monarch-focal-install.sh`n"

# -Wait won't work here, because mintty.exe doesn't really exit until ssh-agent does
start-process C:\cygwin64\bin\mintty.exe -argumentlist "-h always /bin/bash --login /usr/local/src/monarch-focal-install.sh -S $exp_option"
